...
public static partial class NullableExtensions
{
    ...
    /// </returns>
    public static T ValueOr<T>(this T? source, T fallbackValue) where T : class
        => source ?? fallbackValue;

    /// <summary>
    /// Checks if the given instance of <see cref="Nullable{T}"/> has value.
    /// If it's not, then it'll simply return the value.
    /// Otherwise, it'll return specified fallback value.<c>defaultValue</c>.
    /// </summary>
    /// <param name="source">The source possibly null value.</param>
    /// <param name="fallbackValue">The fallback value to be used instead.</param>
    ...
    /// Returns the value itself if it's defined. Otherwise, returns the <c>defaultValue</c>.
    /// </returns>
    public static T ValueOr<T>(this T? source, T fallbackValue) where T : struct
        => source ?? fallbackValue;
    ...
    /// </returns>
    public static T ValueOr<T>(this T? source, Func<T> fallbackValueFactory) where T : class
        => source ?? fallbackValueFactory();
    ...
    /// Otherwise, returns the output of the factory <c>defaultValueFactory</c>.
    /// </returns>
    public static T ValueOr<T>(this T? source, Func<T> fallbackValueFactory) where T : struct
        => source ?? fallbackValueFactory();
}